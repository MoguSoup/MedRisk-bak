...
RENDER_DIR = FINAL_DIR / "render_check_rich"

BASE_DOCX = FINAL_DIR / "唐俊_2026计科生产实习报告册_MedRisk填写版.docx"
OUT_DOCX = FINAL_DIR / "唐俊_2026计科生产实习报告册_MedRisk图文充实版.docx"
...
def add_report_content(doc: Document, anchor) -> None:
    ...
    add_figure(anchor, screenshot("03-disease-predict-form.png"), "图5 疾病预测表单页面")
    add_figure(anchor, screenshot("04-disease-predict-result.png"), "图6 疾病预测结果与解释页面")
    ...
    add_figure(anchor, screenshot("07-knowledge-graph-3d.png"), "图8 医学知识图谱三维可视化")
    ...
    add_figure(anchor, screenshot("06-qa-streaming-answer.png"), "图10 智能问答流式输出页面")
    ...
    add_figure(anchor, screenshot("08-training-management.png"), "图11 模型训练管理页面")
    ...
    add_figure(anchor, screenshot("09-audit-log-ip.png"), "图13 审计日志中的请求 IP 记录")

    add_paragraph(anchor, "2.4 编码测试与结果展示", "h2")
    add_paragraph(
        anchor,
        "编码完成后，我从本地和远端两个环境进行验证。本地 Windows 启动固定使用 conda MedRisk 环境，后端使用 demo profile，模型服务使用 FastAPI，前端使用 Vite。远端 med_tencent 通过 Docker Compose 运行 MySQL、Neo4j、后端、前端和模型服务。由于 80 端口截图时返回 502，最终系统截图使用已经验证可用的 5241 端口完成，并在交付说明中记录入口差异。",
    )
    add_figure(anchor, screenshot("01-login-page.png"), "图14 MedRisk 登录页面")
    add_figure(anchor, screenshot("02-admin-console.png"), "图15 管理员控制台页面")
    add_paragraph(
        anchor,
    ...
    add_figure(anchor, screenshot("10-health-check.png"), "图17 云端后端健康检查结果", width=4.6)
    ...
    add_paragraph(
        anchor,
        "MedRisk 面向医学健康场景，设计时必须考虑社会、健康、安全、法律、文化和隐私影响。系统在页面和报告中说明平台仅用于教学演示和健康风险提示，不能替代医生诊断。邮箱授权码、API Key 和访问 token 不写入源码、报告和文档。智能问答对医学问题展示证据来源，并在证据不足时提示不能作为诊断依据。审计日志记录请求 IP，便于追踪异常行为。平台的积极意义是帮助用户理解风险因素和医学知识，局限是模型输出受训练数据、输入质量和外部模型稳定性影响，需要专业医生参与解释。",
    )
    ...
    reflections = [
        ...
        "医疗健康场景让我更加重视工程伦理。疾病风险预测和智能问答可能会影响用户对自身健康的判断，所以系统必须清楚说明它只能用于教学演示和健康风险提示，不能替代医生诊断。大模型回答也需要给出证据来源和边界提醒。另一方面，系统接入邮箱验证码、阿里云百炼 API 和远端服务器时，会涉及密钥、授权码和访问 token，这些内容不能写入源码和报告。实习过程中我反复检查文档和交付目录，避免泄露真实密钥。这让我认识到安全意识不是最后才补的工作，而应该贯穿设计、编码、测试和文档全过程。",
...
def main() -> None:
    ...
    shutil.copy2(OUT_DOCX, RENDER_DIR / "render_input_rich.docx")
    ...
    summary = (
        ...
        f"- 保密扫描：{'发现疑似敏感模式 ' + ', '.join(secrets) if secrets else '未发现真实密钥、授权码、token 或明文密码模式'}。\n"
    )
    (NOTES_DIR / "rich-report-generation-summary.md").write_text(summary, encoding="utf-8")
    print(OUT_DOCX)
